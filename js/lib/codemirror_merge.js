...
(function(mod) {
  ...
  function getDiff2(a, b) {
    var diff = dmp.diff_main(a, b, true);
  ...
  function getDiff(a, b) {
    ...
    for(var i = 0; i < opcodes.length; i++) {
      var o = opcodes[i];
      var op = o[0];
      var pass = op == 'replace' ? 2 : 1;

      if(pass==2) op = 'delete';
      
      for(var t = 0; t < pass; t++) {
        if(t==1) op = 'insert';
        var c = op == 'delete' ? DIFF_DELETE : ( op == 'insert' ? DIFF_INSERT : DIFF_EQUAL);
        if(op != 'delete') {
          var lines = newtxt;
          var s = o[3]
          var e = o[4];
        ...
        var chunk = "";
        for(var j = s; j < e; j++) {
          var last_line = j == lines.length - 1;
          chunk += lines[j] + (!last_line ? NL : '');
        }
        diff.push([c, chunk]);
      }
    }
